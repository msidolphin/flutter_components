        ...
        if (axis.edgeLabelPlacement == EdgeLabelPlacement.hide) {
          if (axis.labelAlignment == LabelAlignment.center) {
            ...
            continue;
              ...
              (i == 0 ||
                  (i == axis._visibleLabels.length - 1 && axis.isInversed))) {
            axis._visibleLabels[i]._needRender = false;
            continue;
          }
        }
      ...
      location = axis._orientation == AxisOrientation.vertical
          ? (axis.isInversed
              ? ((bounds.top + bounds.height) -
                  ((bounds.top -
                          (bounds.top -
                                  (_valueToCoefficient(value, oldAxis) *
                                      bounds.height))
                              .roundToDouble())
                      .abs()))
              : (bounds.bottom -
                      (_valueToCoefficient(value, oldAxis) * bounds.height))
                  .roundToDouble())
          : (axis.isInversed
              ? ((_valueToCoefficient(value, axis) * bounds.width) +
                      bounds.right)
                  .roundToDouble()
              : ((_valueToCoefficient(value, oldAxis) * bounds.width) -
                      bounds.left)
                  .roundToDouble());
...
class _CartesianAxesPainter extends CustomPainter {
  ...
  void _drawPlotAreaBorder(Canvas canvas) {
    ...
    paint.color = chart.plotAreaBorderColor ??
        chart._chartState._chartTheme.plotAreaBorderColor;
