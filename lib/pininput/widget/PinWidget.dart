...
import '../decoration/PinDecoration.dart';
...
class PinInputTextField extends StatefulWidget {
  ...
  /// The callback will execute when value's length equal than length
  final ValueChanged<String> onCompleted;

  PinInputTextField({
    Key key,
    this.length: _kDefaultPinLength,
    this.onSubmit,
    ...
    this.onCompleted
        ...
        /// length must larger than zero.
        assert(length != null && length > 0),
        assert(decoration != null),

        /// Hint length must equal to the [length].
...
class _PinInputTextFieldState extends State<PinInputTextField> {
  ...
  void _updateText() {
    if (_effectiveController.text.runes.length > widget.length) {
      _text = String.fromCharCodes(
          _effectiveController.text.runes.take(widget.length));
    } else {
  ...
  void didUpdateWidget(PinInputTextField oldWidget) {
    ...
    if (oldWidget.length > widget.length &&
        _text.runes.length > widget.length) {
      setState(() {
...
class PinInputTextFormField extends FormField<String> {
  ...
  PinInputTextFormField({
    ...
    PinDecoration decoration = const UnderlineDecoration(),
    List<TextInputFormatter> inputFormatter,
