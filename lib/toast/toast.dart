...
|     |___________________|       |
|_________________________________|
from https://github.com/MMMzq/bot_toast
 */
...
class Toast {
  ...
  static CancelFunc _cancelFunc = null;

  static void showLoading ({String text}) {
    hideLoading();///如果不取消会造成多个功能页同时调用时，出现无法关闭问题。
    _cancelFunc = _showLoading(text: text);
  }
  ...
  static CancelFunc _showLoading({
    ...
    return showCustomLoading(
        ...
        wrapToastAnimation: wrapToastAnimation,
        align: align,
        toastBuilder: (_) =>  LoadingWidget(locale: locale, text: text,),
        clickClose: clickClose,
