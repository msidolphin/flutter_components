...
class DateUtil {
  ...
  static String getDateStrByTimeStr(
    ...
    return getDateStrByDateTime(getDateTime(dateStr, isUtc),
  ...
  static String formatDateStr(String dateStr, {bool isUtc, String format}) {
    return formatDate(getDateTime(dateStr, isUtc), format: format);
  }
  ...
  static List<int> getPrevMonth(int year, int month) {
    if (month == 1) return [year - 1, 12];
    return [year, month - 1];
  }

  static DateTime getPrevMonthDateTime(int year, int month) {
    int nY = year;
    int nM = month;
    if (month == 1) {
      nY = year - 1;
