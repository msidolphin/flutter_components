...
const double _kHeaderHeight = 42;
...
class SlidingPanel extends StatefulWidget {
  ...
  /// Either SlidePanelDirection.UP or SlidePanelDirection.DOWN. Indicates which way
  ...
  /// down on the panel.
  final SlidePanelDirection slideDirection;
  ...
  SlidingPanel({
    ...
    this.slideDirection = SlidePanelDirection.UP,
    this.defaultPanelState = PanelState.CLOSED,
    this.panelHeader = Gaps.empty,
...
class _SlidingUpPanelState extends State<SlidingPanel> with TickerProviderStateMixin{

  AnimationController _ac;

  AnimationController _showOrHideController;
  Animation<double> _showOrHideTween;

  ScrollController _sc;
  bool _scrollingEnabled = false;
  VelocityTracker _vt = new VelocityTracker();

  bool _isShowOrHiding = false;
  ...
  void initState(){
    ...
    _showOrHideController = new AnimationController(vsync: this,
      duration: const Duration(milliseconds: 200),
    );
    _showOrHideTween = new Tween<double>(
      begin: widget.minHeight,
      end: 0
    ).animate(_showOrHideController);
    _showOrHideController.addListener(() {
      setState(() {
      });
    });
  ...
  Widget build(BuildContext context) {
    return Stack(
      alignment: widget.slideDirection == SlidePanelDirection.UP ? Alignment.bottomCenter : Alignment.topCenter,
      children: <Widget>[
        ...
        Container(
          height: !_isPanelVisible ? 0 : _isShowOrHiding ? _showOrHideTween.value : _ac.value * ((widget.maxHeight + widget.minHeight) - widget.minHeight) + widget.minHeight,
          child: IndexedStack(
            index: !_isPanelVisible ? 0 : 1 ,
            children: <Widget>[
              Container(),
              Container(
                ...
                child: ListView(
                  ...
                  children: <Widget>[
                    ...
                    _gestureHandler(
                      child: Container(
                        height: !_isShowOrHiding ? _ac.value * ((widget.maxHeight) - widget.minHeight) + widget.minHeight : _showOrHideTween.value,
                        ...
                        child: Stack(
                          children: <Widget>[
                            //open panel
                            Positioned(
                                top: widget.slideDirection == SlidePanelDirection.UP ? 0.0 : null,
                                bottom: widget.slideDirection == SlidePanelDirection.DOWN ? 0.0 : null,
                                width:  MediaQuery.of(context).size.width -
...
//        !_isPanelVisible ?  : ,
  ...
  Widget _gestureHandler({Widget child, bool force = false}){
  ...
  double _getParallax(){
    if(widget.slideDirection == SlidePanelDirection.UP)
      return -_ac.value * (widget.maxHeight - widget.minHeight) * widget.parallaxOffset;
    else
      return _ac.value * (widget.maxHeight - widget.minHeight) * widget.parallaxOffset;
  }

  void _onHeaderGestureSlide(double dy) {
    if(widget.slideDirection == SlidePanelDirection.UP)
      _ac.value -= dy / (widget.maxHeight - widget.minHeight);
    else
      _ac.value += dy / (widget.maxHeight - widget.minHeight);
    if (_ac.value > 1) _ac.value = 1;
    if (_ac.value < 0) _ac.value = 0;
    setState(() {
  ...
  void _onGestureEnd(Velocity velocity){
    ...
    if(velocity.pixelsPerSecond.dy.abs() >= minFlingVelocity){
      ...
      if(widget.slideDirection == SlidePanelDirection.DOWN)
  ...
  Future<void> _hide() async {
    if (_isShowOrHiding) return;
    _isShowOrHiding = true;
    return _showOrHideController.forward().then((v) {
      _isPanelVisible = false;
      _isShowOrHiding = false;
      _isHidden = true;
      _isShown = false;
    });
  }
  ...
  Future<void> _show() async {
    ...
    return _showOrHideController.reverse().then((v) {
      _isShowOrHiding = false;
      _isShown = true;
      _isHidden = false;
    });
...
class PanelController{
  ...
  bool get isShown {
    return _panelState._isShown;
  }

  bool get isHidden {
    return _panelState._isHidden;
  }
