...
//      children.insert(3, _buildFormatButton());
  ...
  Widget buildNextCalendar() {
    ...
    int x = 0;
    List<DateTime> visibleDays = widget.calendarController.nextVisibleDays;
    while (x < visibleDays.length) {
      children.add(_buildTableRow(visibleDays
          .skip(x)
          .take(daysInWeek)
          .toList(), widget.calendarController.nextFocusedDay));
      x += daysInWeek;
    }
  ...
  Widget buildPreviousCalendar() {
    ...
    int x = 0;
    List<DateTime> visibleDays = widget.calendarController.previousVisibleDays;
    while (x < visibleDays.length) {
      children.add(_buildTableRow(visibleDays
          .skip(x)
          .take(daysInWeek)
          .toList(), widget.calendarController.previousFocusedDay));
      x += daysInWeek;
    }
  ...
  Widget _buildWrapper({Key key}) {
    Widget wrappedChild = _buildTable(widget.calendarController.focusedDay);
  ...
  Widget _buildHorizontalSwipeWrapper({Widget child}) {
    return AnimatedSwitcher(
      ...
      child: Dismissible(
        ...
        background: buildPreviousCalendar(),
        secondaryBackground: buildNextCalendar(),
        resizeDuration: null,
  ...
  Widget _buildTable(DateTime focusedDay) {
    final daysInWeek = 7;
    final children = <TableRow>[
      if (widget.calendarStyle.renderDaysOfWeek) _buildDaysOfWeek(),
    ];
  ...
  TableRow _buildTableRow(List<DateTime> days, DateTime focusedDay) {
    return TableRow(
      ...
      children: days.map((date) => _buildTableCell(date, focusedDay)).toList(),
    );
  }

  // TableCell will have equal width and height
  Widget _buildTableCell(DateTime date, DateTime focusedDay) {
  ...
  Widget _buildCell(DateTime date, DateTime focusedDay) {
    if (!widget.calendarStyle.outsideDaysVisible &&
        widget.calendarController._isExtraDay(date, focusedDay) &&
        widget.calendarController.calendarFormat == CalendarFormat.month) {
      return Container();
    }
  ...
  Widget _buildCellContent(DateTime date, DateTime focusedDay) {
    ...
    final tIsOutside = widget.calendarController._isExtraDay(date, focusedDay);
